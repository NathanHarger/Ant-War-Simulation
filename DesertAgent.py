    ...
    def runCombat(self, desert):
        ...
        if (len(list_hives_combat) > 1):                   
            ...
            for i in range(len(list_hives_combat)):
                ...
                rand_range = 0.1 * strength #for random spice
                #add random range, then divide by opposing hives                
                strength = strength + r.randint(-rand_range, rand_range)
                ...
                for j in range(len(list_hives_combat)):
                    if (i != j):
                        for k in range(len(list_ants_combat[j])):
                           ...
                           desert.list_hives[list_hives_combat[j]]\
                           .list_ants[list_ants_combat[j]][k].energy = 0
